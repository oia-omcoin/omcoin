...
#if defined(NDEBUG)
# error "Omcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9d3167b8e8fcd39cbfc69949d8fdaa4e27b9d9deb2567b20a7ddb4424421b9d2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Omcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Omcoin Signed Message:\n";
...
{
    // Omcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210240); // Omcoin: 840k blocks in ~2 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Omcoin: 1 hour
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Omcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Omcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9d3167b8e8fcd39cbfc69949d8fdaa4e27b9d9deb2567b20a7ddb4424421b9d2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Birth of Omcoin on 6th of Jan 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x808bf39ce4dea61340879afcb3ca0c4e2bd5e78d71a6599454ed1bbffb611891"));
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc3, 0xb4, 0xd5 }; // Omcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OmcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OmcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OmcoinMiner(CWallet *pwallet)
{
    printf("OmcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("omcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OmcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OmcoinMiner, pwallet));
}
